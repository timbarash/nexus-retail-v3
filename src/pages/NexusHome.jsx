...
const COMMAND_ACTIONS_BY_PERSONA = {
  ceo: [
    { key: 'flash', action: 'daily_flash', label: 'Daily Flash', desc: 'Today vs last week', query: 'Show today\'s flash report across all stores', icon: Activity, color: '#D4A03A' },
    { key: 'margin', action: 'margin_analysis', label: 'Margin Analysis', desc: 'Category COGS breakdown', query: 'Break down gross margins by product category', icon: DollarSign, color: '#00C27C' },
    { key: 'portfolio', action: 'portfolio', label: 'Portfolio P&L', desc: '7-state roll-up', query: 'Give me a portfolio P&L across all 39 stores', icon: Building2, color: '#64A8E0' },
    { key: 'tax', action: 'tax_280e', label: '280E Exposure', desc: 'Tax liability & COGS split', query: 'Show 280E tax exposure and COGS allocation', icon: Receipt, color: '#B598E8' },
    { key: 'catmix', action: 'category_mix', label: 'Category Mix', desc: 'Flower vs edibles shift', query: 'Show category revenue mix and trends', icon: Layers, color: '#0EA5E9' },
    { key: 'shrinkage', action: 'shrinkage', label: 'Shrinkage Report', desc: 'Waste & loss by store', query: 'Show shrinkage and waste across all stores', icon: AlertTriangle, color: '#EC4899' },
  ],
  vp_retail: [
    { key: 'rankings', action: 'rankings', label: 'Store Scorecard', desc: 'Rank all 21 stores', query: 'Rank all stores in my region by composite score', icon: BarChart3, color: '#D4A03A' },
    { key: 'rebalance', action: 'rebalance', label: 'OOS & Rebalance', desc: 'Cross-store transfers', query: 'Which products are out of stock and need rebalancing?', icon: ArrowRightLeft, color: '#E87068' },
    { key: 'vendors', action: 'vendor_scorecard', label: 'Vendor Scorecard', desc: 'Fill rates & on-time %', query: 'Show vendor performance scorecards', icon: Truck, color: '#64A8E0' },
    { key: 'practices', action: 'practices', label: 'Best Practices', desc: 'What top stores do', query: 'What practices from top stores can lift underperformers?', icon: Sparkles, color: '#B598E8' },
    { key: 'online', action: 'online_vs_instore', label: 'Online vs Walk-In', desc: 'Channel mix & AOV', query: 'Show online vs in-store channel performance', icon: Globe, color: '#0EA5E9' },
    { key: 'margin', action: 'margin_analysis', label: 'Margin Deep Dive', desc: 'Where we\'re losing margin', query: 'Where are we losing margin by category?', icon: DollarSign, color: '#EC4899' },
  ],
  regional_mgr: [
    { key: 'transfers', action: 'transfers', label: 'Approve Transfers', desc: '3 pending manifests', query: 'Show pending inter-store transfers needing approval', icon: ArrowRightLeft, color: '#D4A03A' },
    { key: 'close', action: 'close', label: 'Close Reports', desc: 'Cash variances & EOD', query: 'Show yesterday\'s close reports for all IL stores', icon: FileText, color: '#00C27C' },
    { key: 'delivery', action: 'delivery', label: 'Today\'s Deliveries', desc: 'Vendor receiving log', query: 'What vendor deliveries are expected today?', icon: Truck, color: '#64A8E0' },
    { key: 'labor', action: 'labor', label: 'Labor & Overtime', desc: 'Cost per order by store', query: 'Show labor efficiency and overtime across IL stores', icon: Users, color: '#B598E8' },
    { key: 'shrinkage', action: 'shrinkage', label: 'Shrinkage Tracker', desc: 'Waste & loss by store', query: 'Show shrinkage report for Illinois stores', icon: AlertTriangle, color: '#0EA5E9' },
    { key: 'audit', action: 'audit', label: 'Store Readiness', desc: 'Pre-inspection check', query: 'Run store readiness check for upcoming IL inspection', icon: Eye, color: '#EC4899' },
  ],
  store_mgr: [
    { key: 'vault', action: 'vault_transfer', label: 'Vault Pull List', desc: '3 critical SKUs low', query: 'Show vault-to-floor transfer queue for Logan Square', icon: ArrowRightLeft, color: '#E87068' },
    { key: 'budtender', action: 'budtender_perf', label: 'Budtender Stats', desc: 'AOV & upsell by rep', query: 'Show budtender performance for Logan Square', icon: Users, color: '#D4A03A' },
    { key: 'flash', action: 'daily_flash', label: 'Today\'s Sales', desc: 'Real-time vs last week', query: 'How are we tracking today vs last week?', icon: Activity, color: '#00C27C' },
    { key: 'loyalty', action: 'loyalty_health', label: 'Loyalty Program', desc: 'Enrollments & lapsed', query: 'How is our loyalty program doing this month?', icon: Star, color: '#B598E8' },
    { key: 'allotment', action: 'allotment_check', label: 'Allotment Status', desc: 'Customers near limits', query: 'Show customers approaching purchase limits today', icon: Shield, color: '#0EA5E9' },
    { key: 'sentiment', action: 'sentiment_overview', label: 'Customer Feedback', desc: 'Reviews & NPS', query: 'Show customer sentiment and review trends', icon: MessageSquare, color: '#EC4899' },
  ],
  compliance: [
    { key: 'metrc', action: 'metrc_queue', label: 'METRC Queue', desc: 'Untagged & sync errors', query: 'Show METRC package queue and sync errors', icon: Package, color: '#E87068' },
    { key: 'discrepancy', action: 'discrepancy', label: 'Exceptions Queue', desc: '3 urgent mismatches', query: 'Show inventory exceptions sorted by priority', icon: AlertTriangle, color: '#D4A03A' },
    { key: 'sync', action: 'sync_status', label: 'Sync Dashboard', desc: 'All 7 state systems', query: 'Show inventory sync health across all states', icon: RefreshCw, color: '#00C27C' },
    { key: 'audit', action: 'audit', label: 'Readiness Score', desc: 'Pre-inspection check', query: 'Run store readiness check for upcoming IL review', icon: Eye, color: '#64A8E0' },
    { key: 'regulatory', action: 'regulatory', label: 'Reg Changes', desc: '5 upcoming deadlines', query: 'Show upcoming regulatory changes and deadlines', icon: Clipboard, color: '#B598E8' },
    { key: 'shrinkage', action: 'shrinkage', label: 'Shrinkage & Waste', desc: 'Loss by store', query: 'Show shrinkage and waste report for compliance review', icon: FileText, color: '#0EA5E9' },
  ],
};
