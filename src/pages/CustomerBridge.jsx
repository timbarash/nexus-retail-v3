...
/* ═══════════════════════════════════════════════════════════════════
   NEXUS ACTION DATA — operational responses for persona action tiles
   ═══════════════════════════════════════════════════════════════════ */
...
function getNexusActionData(action) {
  const A = {
    ...
    margin_analysis: { title:'Margin Analysis by Category', color:'#00C27C', summary:'Gross margin breakdown across all product categories — identifying margin compression and opportunity.',
      kpis:[{l:'Blended Margin',v:'42.3%',c:'#00C27C'},{l:'Highest',v:'Edibles (48%)',c:'#D4A03A'},{l:'Lowest',v:'Flower (34%)',c:'#E87068'},{l:'vs Last Month',v:'+1.2%',c:'#0EA5E9'}],
      cols:['Category','Revenue','COGS','Margin %','vs Prior','Trend'],
      rows:[['Edibles','$2.1M','$1.09M','48.1%','+0.8%','Balanced'],['Pre-Rolls','$1.8M','$1.01M','43.9%','+2.1%','Balanced'],['Vapes','$2.4M','$1.39M','42.1%','-0.3%','Review'],['Concentrates','$1.2M','$0.74M','38.3%','+0.5%','Balanced'],['Flower','$3.1M','$2.05M','33.9%','-1.1%','Review'],['Tinctures','$0.6M','$0.35M','41.7%','+1.8%','Balanced']],
      actions:[{label:'Flag Low-Margin SKUs',done:'12 SKUs below 30% margin flagged for vendor renegotiation'},{label:'Export for Buyer',done:'Margin report sent to purchasing team'}]},
    shrinkage: { title:'Shrinkage & Waste Report', color:'#E87068', summary:'Inventory shrinkage across all stores — product loss from waste, damage, and unaccounted discrepancies.',
      kpis:[{l:'Total Shrinkage',v:'$42.1K',c:'#E87068'},{l:'% of Revenue',v:'0.38%',c:'#D4A03A'},{l:'Biggest Source',v:'Expired (61%)',c:'#64A8E0'},{l:'vs Target',v:'Over by $8K',c:'#E87068'}],
      cols:['Store','Expired','Damaged','Unaccounted','Total Loss','% Rev'],
      ...
      actions:[{label:'Flag Naperville for Review',done:'Shrinkage audit triggered at Naperville'},{label:'Update Waste SOPs',done:'Updated SOP sent to stores above target'}]},
    tax_280e: { title:'280E Tax Exposure Summary', color:'#D4A03A', summary:'Estimated COGS allocation and non-deductible expense breakdown for 280E planning.',
      kpis:[{l:'Est. Tax Liability',v:'$3.8M',c:'#E87068'},{l:'Effective Rate',v:'~68%',c:'#D4A03A'},{l:'COGS Allocated',v:'$6.2M',c:'#00C27C'},{l:'Potential Savings',v:'$410K',c:'#0EA5E9'}],
      cols:['Expense Category','Annual Amount','Deductible?','COGS Allocable','Notes'],
      rows:[['Product Purchases','$48.2M','Yes (COGS)','100%','Direct cost'],['Rent (store floor)','$4.1M','Partial','~60%','Selling space ratio'],['Budtender Wages','$6.8M','Partial','~70%','Direct sales labor'],['Security','$1.9M','Partial','~40%','Vault/product security'],['Compliance Staff','$1.2M','No','0%','Non-COGS overhead'],['Marketing','$2.4M','No','0%','280E non-deductible']],
      actions:[{label:'Generate 280E Worksheet',done:'Worksheet exported for CPA review'},{label:'Schedule CPA Review',done:'Meeting scheduled with tax advisor — Apr 3'}]},
    category_mix: { title:'Category Mix & Trends', color:'#B598E8', summary:'Product category revenue share and movement trends across the portfolio.',
      kpis:[{l:'Top Category',v:'Flower (28%)',c:'#00C27C'},{l:'Fastest Growing',v:'Pre-Rolls (+22%)',c:'#0EA5E9'},{l:'Declining',v:'Vapes (-3%)',c:'#E87068'},{l:'Categories',v:'8',c:'#B598E8'}],
      ...
      actions:[{label:'Adjust Shelf Space',done:'Planogram update sent — more pre-roll space allocated'},{label:'Flag Vape Decline',done:'Vape category review assigned to merchandising'}]},
    budtender_perf: { title:'Budtender Performance — Logan Square', color:'#00C27C', summary:'Individual budtender metrics for this store this month.',
      kpis:[{l:'Top Seller',v:'Maria G.',c:'#00C27C'},{l:'Avg AOV',v:'$97.40',c:'#D4A03A'},{l:'Attach Rate',v:'2.4 items',c:'#0EA5E9'},{l:'Staff',v:'16',c:'#B598E8'}],
      ...
      actions:[{label:'Create Training Plan',done:'Coaching sessions scheduled for bottom 3 budtenders'},{label:'Reward Top Sellers',done:'Bonus nominations submitted for Maria & James'}]},
    daily_flash: { title:'Daily Flash Report — Today', color:'#D4A03A', summary:'Real-time sales snapshot across the portfolio for today so far.',
      kpis:[{l:'Today So Far',v:'$198K',c:'#00C27C'},{l:'vs Same Day LW',v:'+8.2%',c:'#0EA5E9'},{l:'Orders',v:'1,847',c:'#D4A03A'},{l:'Avg Basket',v:'$107.20',c:'#B598E8'}],
      cols:['Store','Revenue','Orders','AOV','vs Last Week','Peak Hour'],
      rows:[['River North','$32.1K','298','$107.70','+11%','12-1pm'],['Logan Square','$28.4K','264','$107.60','+7%','11am-12pm'],['Ann Arbor','$24.8K','231','$107.40','+9%','1-2pm'],['Columbus','$22.1K','206','$107.30','+5%','12-1pm'],['Edison','$21.6K','198','$109.10','+12%','11am-12pm'],['Naperville','$18.9K','178','$106.20','+3%','12-1pm']],
      actions:[{label:'Send to Leadership',done:'Flash report emailed to exec team'},{label:'Set Alerts',done:'Alerts set for stores trending below -5% vs LW'}]},
    allotment_check: { title:'Purchase Allotment Status', color:'#64A8E0', summary:'Customers approaching daily purchase limits — flagged for budtender awareness.',
      kpis:[{l:'Near Limit',v:'23',c:'#D4A03A'},{l:'At Limit',v:'4',c:'#E87068'},{l:'Avg Utilization',v:'62%',c:'#0EA5E9'},{l:'Med Patients',v:'8 near',c:'#B598E8'}],
      ...
      actions:[{label:'Alert Budtenders',done:'Allotment warnings sent to POS terminals'},{label:'Export for Records',done:'Daily allotment log saved'}]},
    vendor_scorecard: { title:'Vendor Scorecard — Top Suppliers', color:'#B598E8', summary:'Vendor performance based on fill rates, delivery reliability, and margin contribution.',
      ...
      actions:[{label:'Flag Cookies for Review',done:'Performance review meeting requested with Cookies rep'},{label:'Send Scorecards',done:'Quarterly scorecards sent to all 24 vendors'}]},
    online_vs_instore: { title:'Online vs In-Store Mix', color:'#0EA5E9', summary:'Channel performance comparison — ecommerce pickup, delivery, and walk-in traffic.',
      ...
      actions:[{label:'Boost Online Pickup',done:'15% off first online order promo activated'},{label:'Optimize Delivery Zones',done:'Delivery radius analysis sent to ops team'}]},
    loyalty_health: { title:'Loyalty Program Health', color:'#00C27C', summary:'Loyalty enrollment, engagement, and redemption metrics across the portfolio.',
      kpis:[{l:'Total Members',v:'48.2K',c:'#00C27C'},{l:'Active (30d)',v:'18.7K',c:'#0EA5E9'},{l:'Redemption Rate',v:'34%',c:'#D4A03A'},{l:'Lift vs Non-Loyalty',v:'+28% AOV',c:'#B598E8'}],
      ...
      actions:[{label:'Re-engage Lapsed',done:'Win-back SMS sent to 4,218 lapsed members'},{label:'Boost Enrollments',done:'Double-points promo activated for new sign-ups'}]},
    metrc_queue: { title:'METRC Package Queue', color:'#D4A03A', summary:'Packages awaiting METRC tagging, pending manifests, and sync issues.',
      ...
      actions:[{label:'Bulk Tag All',done:'14 packages tagged in METRC — labels printing'},{label:'Resolve Sync Errors',done:'2 sync errors cleared — packages now tracked'}]},
  };
...
export default function CustomerBridge({ compact = false, nexusOverlay = false, mobileCompact = false }) {
  ...
  const NEXUS_SUGGESTIONS_BY_PERSONA = {
    ceo: [
      { key: 'flash', label: 'How are we tracking today vs last week?', icon: Activity, gradient: 'from-amber-600/20 to-yellow-600/20', border: 'hover:border-amber-500/40', tag: 'Flash', tagColor: '#D4A03A', action: 'daily_flash' },
      { key: 'margin', label: 'Where are we losing margin by category?', icon: DollarSign, gradient: 'from-green-600/20 to-emerald-600/20', border: 'hover:border-green-500/40', tag: 'Margin', tagColor: '#00C27C', action: 'margin_analysis' },
      { key: 'tax', label: 'What\'s our 280E tax exposure this quarter?', icon: Receipt, gradient: 'from-purple-600/20 to-violet-600/20', border: 'hover:border-purple-500/40', tag: '280E', tagColor: '#B598E8', action: 'tax_280e' },
      { key: 'shrinkage', label: 'Which stores have the highest shrinkage?', icon: AlertTriangle, gradient: 'from-red-600/20 to-rose-600/20', border: 'hover:border-red-500/40', tag: 'Shrinkage', tagColor: '#E87068', action: 'shrinkage' },
    ],
    vp_retail: [
      ...
      { key: 'vendors', label: 'Which vendors are shorting us on fill rates?', icon: Truck, gradient: 'from-blue-600/20 to-cyan-600/20', border: 'hover:border-blue-500/40', tag: 'Vendors', tagColor: '#64A8E0', action: 'vendor_scorecard' },
      { key: 'online', label: 'How\'s online order volume vs walk-in?', icon: Globe, gradient: 'from-amber-600/20 to-yellow-600/20', border: 'hover:border-amber-500/40', tag: 'Channels', tagColor: '#D4A03A', action: 'online_vs_instore' },
    ...
    regional_mgr: [
      { key: 'transfers', label: 'Any inter-store transfers waiting on me?', icon: ArrowRightLeft, gradient: 'from-amber-600/20 to-yellow-600/20', border: 'hover:border-amber-500/40', tag: 'Transfers', tagColor: '#D4A03A', action: 'transfers' },
      { key: 'close', label: 'Any cash variances from last night\'s close?', icon: FileText, gradient: 'from-green-600/20 to-emerald-600/20', border: 'hover:border-green-500/40', tag: 'Close', tagColor: '#00C27C', action: 'close' },
      { key: 'delivery', label: 'What vendor deliveries are coming in today?', icon: Truck, gradient: 'from-blue-600/20 to-cyan-600/20', border: 'hover:border-blue-500/40', tag: 'Receiving', tagColor: '#64A8E0', action: 'delivery' },
      { key: 'shrinkage', label: 'Which stores have unusual waste or shrinkage?', icon: AlertTriangle, gradient: 'from-red-600/20 to-rose-600/20', border: 'hover:border-red-500/40', tag: 'Shrinkage', tagColor: '#E87068', action: 'shrinkage' },
    ],
    store_mgr: [
      { key: 'vault', label: 'What needs to come out of the vault right now?', icon: ArrowRightLeft, gradient: 'from-red-600/20 to-rose-600/20', border: 'hover:border-red-500/40', tag: 'Vault', tagColor: '#E87068', action: 'vault_transfer' },
      { key: 'budtender', label: 'How are my budtenders performing on AOV?', icon: Users, gradient: 'from-amber-600/20 to-yellow-600/20', border: 'hover:border-amber-500/40', tag: 'Staff', tagColor: '#D4A03A', action: 'budtender_perf' },
      { key: 'allotment', label: 'Any customers close to their daily limit?', icon: Shield, gradient: 'from-blue-600/20 to-cyan-600/20', border: 'hover:border-blue-500/40', tag: 'Allotment', tagColor: '#64A8E0', action: 'allotment_check' },
      { key: 'loyalty', label: 'How\'s our loyalty program doing this month?', icon: Star, gradient: 'from-purple-600/20 to-violet-600/20', border: 'hover:border-purple-500/40', tag: 'Loyalty', tagColor: '#B598E8', action: 'loyalty_health' },
    ],
    compliance: [
      { key: 'metrc', label: 'Any METRC packages untagged or stuck in sync?', icon: Package, gradient: 'from-red-600/20 to-rose-600/20', border: 'hover:border-red-500/40', tag: 'METRC', tagColor: '#E87068', action: 'metrc_queue' },
      { key: 'discrepancy', label: 'Show me urgent inventory exceptions', icon: AlertTriangle, gradient: 'from-amber-600/20 to-yellow-600/20', border: 'hover:border-amber-500/40', tag: 'Exceptions', tagColor: '#D4A03A', action: 'discrepancy' },
      { key: 'audit', label: 'Are we ready for the upcoming IL inspection?', icon: Eye, gradient: 'from-blue-600/20 to-cyan-600/20', border: 'hover:border-blue-500/40', tag: 'Readiness', tagColor: '#64A8E0', action: 'audit' },
      { key: 'regulatory', label: 'What reg changes are coming I need to prep for?', icon: Clipboard, gradient: 'from-purple-600/20 to-violet-600/20', border: 'hover:border-purple-500/40', tag: 'Upcoming', tagColor: '#B598E8', action: 'regulatory' },
    ],
  };
